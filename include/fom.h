...
public:
  ...
  fom(double uncertain,fom_type type){ unc = uncertain; fomType = type;}
  //destructor
  ~fom(){};
  //set bkg
  ...
  double maxSignificance(TH1D* sig, TH1D* bkg, bool info, double min_signal = 3.4, TH1D* cuteff = 0);
  float  maxSignificance(TH1F* sig, TH1F* bkg, bool info, float  min_signal = 3.4, TH1F* cuteff = 0);
